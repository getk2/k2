...
$isATagView = ($view=='tags' || $view=='tag');

if(K2_JVERSION=='15') {
    if(($params->get('lockTags') && $user->gid<=23 && $isATagView) || ($user->gid <= 23) && $isAnAdminView)
    {
    ...
    if(
    	($params->get('lockTags') && !$user->authorise('core.admin', 'com_k2') && $isATagView) ||
        (!$user->authorise('core.admin', 'com_k2')) && $isAnAdminView
    )
    {
        JError::raiseError( 403, JText::_('K2_ALERTNOTAUTH') );
    }
