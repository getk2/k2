				...
				<tbody>
					...
					<tr class="row<?php echo ($key%2); ?>"<?php if($this->filter_featured!='1') echo ' sortable-group-id="'.$row->catid.'"'; ?>>
						...
						<td>
							...
							<a class="k2ListItemDisabled" title="<?php echo JText::_('K2_CLICK_TO_ADD_THIS_ENTRY'); ?>" onclick="window.parent.jSelectItem('<?php echo $row->id; ?>', '<?php echo str_replace(array("'", "\""), array("\\'", ""), $row->title); ?>', '<?php echo JRequest::getCmd('fid'); ?>', '<?php echo JRequest::getVar('fname'); ?>', '<?php echo JRequest::getCmd('output'); ?>'); return false;">
								<?php echo $row->title; ?>
							</a>
							<?php else: ?>
