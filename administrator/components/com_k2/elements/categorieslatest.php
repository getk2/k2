        ...
        $document->addScriptDeclaration("
			function jSelectCategory(id, title, object) {
				...
				\$K2('#categoriesList input').each(function(){
					if(\$K2(this).val()==id){
						\$K2().k2Alert('".JText::_('K2_THE_SELECTED_CATEGORY_IS_ALREADY_IN_THE_LIST')."'.replace('CATEGORY_NAME_HERE', title), 3000);
						exists = true;
					}
				});
				if(!exists){
					...
					\$K2().k2Alert('".JText::_('K2_CATEGORY_ADDED_IN_THE_LIST')."'.replace('CATEGORY_NAME_HERE', title), 1000);
				}
			}
