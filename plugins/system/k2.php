...
class plgSystemK2 extends JPlugin
{
    public function onAfterInitialise()
    {
        ...
        $this->initialiseVersionDefines();
        $this->generalInitialisation($application, $params);

        if ($application->isAdmin()) {
            if ($params->get('gatherStatistics', 1)) {
                $this->metricsInitialisation($application, $params);
            }
            $this->joomFishIntegrationInitialisation($application, $params);
        }
    }

    private function generalInitialisation($application, $params)
    {
        ...
        JResponse::setHeader('X-Logged-In', ($user->guest ? 'False' : 'True'), true);
    ...
    private function metricsInitialisation($application, $params)
    {
        // K2 Metrics
    ...
    private function joomFishIntegrationInitialisation($application, $params)
    {
        $oldJ = ((int) K2_JVERSION < 25);

        $option = $oldJ ? JRequest::getCmd('option') : JFactory::getApplication()->input->get('option');

        if ($option !== 'com_joomfish') {
            return;
        }

        $task = $oldJ ? JRequest::getCmd('task') : JFactory::getApplication()->input->get('task');
    ...
    /* ============= Helper Functions ============= */
    /* ============================================ */
    private function getSearchValue($id, $currentValue)
    {
        JTable::addIncludePath(JPATH_ADMINISTRATOR . '/components/com_k2/tables');
    ...
    private function renderOriginal($extraField, $itemID)
    {
        $application = JFactory::getApplication();
        JTable::addIncludePath(JPATH_ADMINISTRATOR . '/components/com_k2/tables');
        $item = JTable::getInstance('K2Item', 'Table');
    ...
    private function renderTranslated($extraField, $itemID)
    {
        $application = JFactory::getApplication();
        JTable::addIncludePath(JPATH_ADMINISTRATOR . '/components/com_k2/tables');
        $item = JTable::getInstance('K2Item', 'Table');
        ...
        return $this->extraFieldHTML($extraField, $defaultValues, $active);
    }

    private function extraFieldHTML($extraField, $defaultValues, $active)
    {
        $strongName = '<strong>' . $extraField->name . '</strong>';
        $fieldName = 'K2ExtraField_'.$extraField->id;
        switch ($extraField->type) {
            case 'textfield' :
                return '<div>'.$strongName.'<br /><input type="text" name="'.$fieldName.'" value="' . $active . '" /></div><br /><br />';
            case 'textarea' :
                return '<div>'.$strongName.'<br /><textarea name="'.$fieldName.'" rows="10" cols="40">' . $active . '</textarea></div><br /><br />';
            case 'select' :
                return '<div style="display:none;">' . JHTML::_('select.genericlist', $defaultValues, $fieldName, '', 'value', 'name', $active) . '</div>';
            case 'multipleSelect' :
                return '<div style="display:none;">' . JHTML::_('select.genericlist', $defaultValues, $fieldName.'[]', 'multiple="multiple"', 'value', 'name', $active) . '</div>';
            case 'radio' :
                return '<div style="display:none;">' . JHTML::_('select.radiolist', $defaultValues, $fieldName, '', 'value', 'name', $active) . '</div>';
            case 'link' :
                return '<div>'.$strongName.'<br /><input type="text" name="'.$fieldName.'[]" value="' . $active[0] . '" /><br /><input type="hidden" name="'.$fieldName.'[]" value="' . $active[1] . '" /><br /><input type="hidden" name="'.$fieldName.'[]" value="' . $active[2] . '" /></div><br /><br />';
        }
        return '';
    }
    ...
    private function initialiseVersionDefines()
    {
        ...
        define('K2_CURRENT_VERSION', '2.8.1');
